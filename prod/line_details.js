...
// INIT MAP
let mymap = L.map('map').setView([45.188529, 5.724524], 13);
let geo;
...
// GENERATE LIGNE
const map = () => $.ajax({
    url: `http://data.metromobilite.fr/api/lines/json?types=ligne&codes=${id}`,
    type: "GET",
    dataType: "json",
}).done((data) => {
...
// FERME LA CARTE (CLICK)
$('#close_modal').click(() => {
    $('#map_modal').css('visibility', 'hidden');
})

// FERME LA CARTE (ECHAP)
$(document).keydown((e) => {
    if (e.keyCode === 27) {
        $('#map_modal').css('visibility', 'hidden');
    }
